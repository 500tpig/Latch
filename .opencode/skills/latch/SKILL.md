...
5. 多 agent 并行时，接入项目应给每个 agent 设置稳定的 `LATCH_ACTOR`。`LATCH_ACTOR` 是唯一可靠身份来源；自动识别只能判断工具来源，不要让多个 AI 共用 `claude:default`、`opencode:default` 或 `unknown:default`。
