...
- For schema 5 submission, prefer `--knowledge-impact-none <reason>` for a concrete `none` record. Use `--knowledge-impact-file` for `updated` and other structured impacts with artifact references. Attach every `knowledge_impact.updated` `artifact_refs` entry to the task before submit. Submit reports every missing reference in one response and returns `latch artifact add <task-id> --expect-revision <revision> <kind:path>...`.
