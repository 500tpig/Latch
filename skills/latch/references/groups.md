...
- Use `checkpoint --group <id>` only with the exact user-supplied Group ID; it stores the existing `group_id` atomically and never infers membership from scope overlap, titles, or neighboring tasks.
...
- Grouped `done` / `abandon` JSON may report a bounded read-only summary of remaining open siblings by phase and task ID. This summary does not change `next_action` or grant sibling takeover, approval, verification, or archive authority.
