...
- Treat CLI version `0.3.0` as the minimum writer for current schema 3 workspace scope, proof, and verification evidence. Version `0.2.0` is only the pre-proof schema 3 baseline and must not write tasks containing the new fields.
...
- Run `latch downgrade-v2 --task <id> --expect-revision <n> --confirm-data-loss --json` only after explicit user confirmation that v3-only fields and event details will remain only in backup. The schema 2 main projection strips `workspace_scope`, `workspace_proof`, proof generation, violations, and evidence refs from plan, verification, and events.
