...
- Treat schema 3 as read-only. Continue a named open schema 3 task only after explicit authorization, then run `latch upgrade-v4 --task <task-id> --expect-revision <n> --json` as its current primary writer. This preserves plan/work revisions, approval, proof generation, verification, and evidence refs.
