...
- 记录用户对 plan 的实施批准；
- 保护同一 task 的并发状态写入；
- 限制同一 Git worktree 只有一个 implementation task；
- 执行并记录命名验证项；
- 提交当前实现供用户 review；
- 记录用户反馈和返工轮次；
- 用户明确认可后归档；
...
- 不改变当前 phase；
- 不自动 abandon；
- blocked 的 dev/check/review task 仍占用当前 worktree；
...
## 11. 同一 worktree 的开发占用

同一个 `workspace_root` 同时只允许一张 task 处于：

```text
dev | check | review
```

review 继续占用，因为 Latch 不要求 clean worktree、不追踪文件归属，也不自动 commit。blocked 不释放占用，因为实现改动仍可能留在 worktree。

进入 dev 前必须持有 workspace 级短锁，并在同一临界区内完成：

```text
扫描 open task
→ 检查 dev/check/review 占用
→ 修改目标 task
```

这样可以避免两张 task 并发 approve 时同时通过扫描。

任务进入 plan、done 或 abandoned 后释放逻辑占用。真正并行实现需要外部 Git worktree；v2 不创建、合并或删除 worktree。

该规则只限制 Latch phase，不证明 Git diff 已隔离。
...
- 在 workspace 短锁内检查当前 workspace 没有其它 dev/check/review task；
...
- 不同 task 可以并行写状态；
- 两张 task 并发 approve 时只有一张成功；
- 同一 workspace 同时只有一张 dev/check/review task；
- blocked 不释放实现占用；
