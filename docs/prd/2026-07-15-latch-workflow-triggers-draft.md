# 触发、判定表、反馈与 worktree（最终契约草案节）

Source-Task: 20260714084358411-重审-latch-最终任务与知识上下文设计-51d5e1

Decision-Status: draft (candidate r1; fills suspended references from Light/Group)
...
Revised: 2026-07-15 — provenance 唯一真源 task.json；冲突与重置规则写死。
...
最终契约 **取代** v2/AGENTS/skill 中「只有用户说出 Latch 才建 task」的规则。

- **发布时**须同步改 `AGENTS.md`、`skills/latch/SKILL.md`、`DESIGN.md` 相关句（见迁移章发布边界）；
- 在 current 文档切换完成前，仓库仍以 v2 显式触发为准（避免文档超前代码/ skill）。
