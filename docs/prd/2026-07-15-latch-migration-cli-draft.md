# 迁移、CLI 总表、兼容回退与发布边界（最终契约草案节）

Source-Task: 20260714084358411-重审-latch-最终任务与知识上下文设计-51d5e1

Decision-Status: draft (candidate r2 — P1 rollback + v2 base + release boundary)
...
Revised: 2026-07-15 — 收紧 schema/R2 映射；全面 current 含 S4/S5 与 AI_INSTALL.md。  
Revised: 2026-07-15 — events_meta 仅首行至多一条；R2 事件 revision 唯一重写为 1..n。

## 1. 与 v2 的关系（权威）

```text
基础契约 = docs/prd/2026-07-10-latch-v2.md（直至 INDEX 切换）
最终契约 = v2 全文保留条款
            + 本最终分章按「同名主题优先级覆盖」
```
...
切换 INDEX current 之前：实现与 skill **仍以 v2 为准**；最终分章仅设计真源。
...
`AI_INSTALL.md` 当前仍描述 v2 命令与「业务项目 AGENTS 只保留显式 Latch 入口」——在切换触发规则或宣称全面 current 时**必须**同发修订，否则视为发布不合格。
...
缺 C8 的 AI_INSTALL/AGENTS 仍写「仅显式 Latch」而代码已自动建 light → **阻断发布**。

实施与切换：**另开 task 并 approve**；本设计 task 不自动执行。
...
| S6 | 迁移 + R2 降级 |
| S7 | 全面 current 文档/指令面（仅当 §7.3 满足） |
| S8 | 观察期 |
