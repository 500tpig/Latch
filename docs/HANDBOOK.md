...
| `latch finish ...` | 一次写完收尾信息 | 在 `finish` 阶段可用；如果当前在 `check` 且 verify 已通过，会自动进入 `finish`。可同时写 closure、知识记忆判断和 artifacts。closure 写进 `task.json` 结构化字段（真源，`context --json --brief` 带出，最后 finish 覆盖），`notes.md` scaffold 是人读副本。 |
...
| `latch list` | 查看未归档任务 | 列出 `.latch/tasks/` 下的任务，`*` 标出当前 actor 的 current，并显示 owner。finish 阶段标 `[wait]`（等用户确认，可直接 done）或 `[blocked]`（卡门禁）。 |
...
- 每个 agent 要有稳定的 actor 标识。`LATCH_ACTOR` 是唯一可靠身份来源，CLI 会原样使用它。没有显式设置时，CLI 会尽力自动识别：`CODEX_THREAD_ID` 会写成 `codex:default:<thread-id>`；存在任意 `CLAUDE_CODE_*` 环境变量会写成 `claude:default`；存在任意 `OPENCODE_*` 环境变量会写成 `opencode:default`；都没有时写成 `unknown:default`。自动识别只能判断工具来源，不能稳定区分具体 agent 或 session，也无法区分「工具本身」和「工具作为子进程启动」——OpenCode 当前不暴露任何 `OPENCODE_*` 环境变量，嵌套在 Claude Code CLI 下跑时会继承父进程的 `CLAUDE_CODE_*` 变量，被误判为 `claude:default`。想让看板稳定区分 agent/session，或要在嵌套环境里正确归属，就设置 `LATCH_ACTOR`，例如 `codex:default:<thread-id>`、`claude:planner:<session>`、`opencode:default:<run-id>`。
...
| `notes.md` | 人主读、AI 兜底读的过程记录、阶段模板和 closure 副本。closure 真源在 `task.json`。 |
...
- finish closure 已写清改了什么、验证了什么、没验证什么、下次接什么（`task.json` 结构化字段为真源，`notes.md` scaffold 为人读副本）；没有未覆盖范围时写「无」。纯文档/commit 跳级任务要写清未验证范围。CLI 不解析 `notes.md` 判断 closure 写得好不好。
- 用户明确确认完成、收尾或归档。
...
- closure 四字段已在 `context --json --brief` 里，续接 finish/done 任务不用读 `notes.md` 拿 closure。
- 不用完整 diff 替代任务摘要。
