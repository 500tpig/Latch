...
输入按首次出现顺序去重；绝对路径、repo escape、glob、Git pathspec magic、repo root、
现存目录缺少尾斜杠以及没有新增路径的调用均在 mutation 前返回
...
`append-scope` 是唯一的 scope-only plan delta。它只追加 machine scope；未携带结构化
