...
本文件只回答一件事：什么时候只留 Latch 记录，什么时候要额外产出给人看的正式文档。
...
`notes.md` 的兜底读时机：当 `context --json --brief` 的 `recent_events` 指向未展开内容，或 `progress.next_action` 卡住时，AI 再读 `notes.md` 取细节；顺利路径下不必读。

## 什么时候只留 Latch 记录

进入正式任务后，满足任一情况时，只用 `task.json + notes.md` 就够：
...
这类任务如有复用价值，再额外生成知识卡；没有就只保留任务记录。未进入正式任务的小修用 `latch log` 留一行。

## 什么时候补讨论摘记

满足任一情况时，额外补一段「讨论摘记」：

- 用户回答了会影响范围、验收、下一步的问题
- 明确采纳或否掉了某个建议
- 重新划定了不做什么
- 讨论结果需要给后续实现、测试或复盘直接引用

小任务把讨论摘记写在当前 task 的 `notes.md`；中等任务直接写进 brief。讨论摘记不追求逐字记录，只保留问题、用户确认、采纳/不采纳、结论和未决点。
...
- 已进入 Latch 的小任务：`task.json` + `notes.md`，不加正式文档
- 有关键规划问答的小任务：`task.json` + `notes.md`，并补讨论摘记
- 未进入 Latch 的小修：`latch log` 留一行
- 中任务：Latch + brief
- 大任务：Latch + PRD
...
- full JSON 输出（`resume --json`、`context --json`、`list --json`）会带出 `artifacts`；brief JSON 默认省略。人读输出里也有一行 `Artifacts:`。
